...
@section('content')
    ...
    <style>
        ...
        {{-- HEADER HALAMAN & TOMBOL SIMPAN KE ATAS --}}
        ...
        {{-- BANNER NOTIFIKASI ALERTS MODERN --}}
        @if(session('sukses'))
                ...
                <form action="/presensi/manual" method="GET">
                    <div class="row g-2 align-items-center">
                        ...
                        @if(auth()->user()->role == 'admin')
                            ...
                            <div class="col-md-4">
                                <input type="date" name="tanggal" class="form-control rounded-3 p-2 shadow-none border" style="font-size: 0.9rem;" value="{{ request('tanggal', date('Y-m-d')) }}">
                            </div>
                            <div class="col-md-3">
            ...
            <div class="card border-0 shadow-premium rounded-4 bg-white p-3 h-100 position-relative overflow-hidden">
                <div class="card-body p-0 d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <h3 class="fw-bold text-success mb-0 font-monospace fs-2" style="letter-spacing: -1px;">{{ $jumlahHadir }}</h3>
                    </div>
            ...
            <div class="card border-0 shadow-premium rounded-4 bg-white p-3 h-100 position-relative overflow-hidden">
                <div class="card-body p-0 d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <h3 class="fw-bold text-primary mb-0 font-monospace fs-2" style="letter-spacing: -1px;">{{ $jumlahSakit }}</h3>
                    </div>
            ...
            <div class="card border-0 shadow-premium rounded-4 bg-white p-3 h-100 position-relative overflow-hidden">
                <div class="card-body p-0 d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <h3 class="fw-bold text-warning mb-0 font-monospace fs-2" style="letter-spacing: -1px;">{{ $jumlahIzin }}</h3>
                    </div>
            ...
            <div class="card border-0 shadow-premium rounded-4 bg-white p-3 h-100 position-relative overflow-hidden">
                <div class="card-body p-0 d-flex align-items-center justify-content-between">
                    <div>
                        ...
                        <h3 class="fw-bold text-danger mb-0 font-monospace fs-2" style="letter-spacing: -1px;">{{ $jumlahAlpa }}</h3>
                    </div>
            ...
            <form action="{{ url('/presensi/manual/simpan') }}" method="POST" id="formPresensi">
                    ...
                    <table class="table align-middle m-0 table-custom-hover">
                        ...
                        <tbody>
                            @if(isset($semua_siswa) && $semua_siswa->count() > 0)
                                ...
                                @endphp
                                <tr class="baris-siswa" data-status-aktif="{{ $statusAwal }}">
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2.5 text-truncate">
                                            @if($siswa->foto)
                                                <img src="{{ asset('storage/'.$siswa->foto) }}" style="width:36px; height:36px; border-radius:50%; object-fit:cover;" class="border" alt="Foto">
                                            @else
                                                <img src="{{ asset('assets/img/default-user.png') }}" style="width:36px; height:36px; border-radius:50%; object-fit:cover;" class="border" alt="Default">
                                            @endif
                                            <div class="overflow-hidden">
                                                <div class="fw-bold text-dark text-uppercase text-truncate mb-0.5" style="font-size: 0.84rem; letter-spacing: 0.2px;" data-search="{{ strtolower($siswa->nama_siswa) }} {{ $siswa->nisn }}">{{ $siswa->nama_siswa }}</div>
                                                <div class="text-muted font-monospace" style="font-size:0.75rem;">NISN: {{ $siswa->nisn }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        <input type="text" name="presensi[{{ $siswa->id }}][keterangan]" class="form-control form-control-sm rounded-3 shadow-none border" value="{{ $siswa->keterangan_hari_ini }}" placeholder="Contoh: Demam, Izin Keluarga" style="width:100%; max-width:240px; font-size: 0.82rem; padding: 6px 10px;">
                                    </td>
                                </tr>
                                @endforeach
                            @else
                ...
                <div class="card-footer bg-white p-3 px-4 d-flex justify-content-between align-items-center border-top">
                    <div class="text-muted small fw-semibold">
                        Menampilkan <span id="count-terfilter">{{ $semua_siswa->count() }}</span> dari <span>{{ $semua_siswa->count() }}</span> total keseluruhan siswa kelas
                    </div>
                </div>
                @endif
            </form>
