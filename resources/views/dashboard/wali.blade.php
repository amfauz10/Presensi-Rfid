{{-- ==========================================
      DASHBOARD WALI KELAS (SAAS PREMIUM STYLE)
     ========================================== --}}
    <style>
    ...
    {{-- 1 & 2. HEADER DASHBOARD & BADGE KANAN --}}
    <div class="mb-4 d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 border-bottom border-light pb-3 text-start">
        ...
        <span class="badge bg-white text-success border border-success-subtle px-3 py-2 rounded-pill fw-semibold font-monospace small d-inline-flex align-items-center shadow-sm">
            <i class="bi bi-mortarboard me-1.5"></i> Kelas {{ auth()->user()->kelas->nama_kelas }}
        </span>
    </div>
        ...
        <div class="col-12">
            <div class="card-custom text-start position-relative overflow-hidden">
                <div class="d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3 mb-4">
                    <div>
                        <h5 class="fw-bold m-0 text-dark" style="font-size: 0.95rem;">Tren Kehadiran Kelas</h5>
    ...
    <div class="row g-4 text-start mb-4">
        {{-- 5. RINCIAN STATUS KEHADIRAN HARI INI --}}
        <div class="col-lg-5">
            <div class="card-custom h-100 p-0 overflow-hidden">
                <div class="p-3 px-4 border-bottom bg-white">
                    <h5 class="fw-bold m-0 text-dark" style="font-size: 0.95rem;">Rincian Status Hari Ini</h5>
                </div>
                <div class="p-4">
                    @php
                        $rincianStatus = [
                    ...
                    @endphp
                    @foreach ($rincianStatus as $i => $r)
                        <div class="d-flex justify-content-between align-items-center mb-1.5">
                            <span class="text-dark fw-medium" style="font-size: 0.85rem;">
                                <span class="d-inline-block rounded-circle me-2" style="width:6px; height:6px; background-color:{{ $r['warna'] }};"></span>
                                {{ $r['label'] }}
                            </span>
                            <span class="fw-bold font-monospace text-slate-700" style="font-size: 0.875rem;">{{ $r['jumlah'] }}</span>
                        </div>
                        <div class="progress mb-3">
                            <div class="progress-bar" style="width:{{ $totalSiswa ? round(($r['jumlah'] / $totalSiswa) * 100) : 0 }}%; background-color:{{ $r['warna'] }};"></div>
                        </div>
                    @endforeach
                </div>
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // ====================================================
        // GRAFIK 1: TREN KEHADIRAN KELAS (Stacked Bar Chart — v1.1)
        // ====================================================
