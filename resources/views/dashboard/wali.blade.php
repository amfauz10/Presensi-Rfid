{{-- ==========================================
      DASHBOARD WALI KELAS (OPTIMIZED FROM ADMIN)
     ========================================== --}}
    <style>
    ...
    <div class="mb-4 d-flex justify-content-between align-items-center border-bottom border-light pb-3">
        ...
        <span class="badge bg-success px-3 py-2 rounded-pill fw-semibold small" style="box-shadow: 0 4px 12px rgba(22, 163, 74, 0.15) !important;">
            <i class="bi bi-mortarboard me-1"></i> Kelas {{ auth()->user()->kelas->nama_kelas }}
        </span>
    </div>
                    ...
                    @foreach ($rincianStatus as $i => $r)
                        <div class="d-flex justify-content-between align-items-center {{ !$loop->last ? 'mb-3' : '' }}">
                            <span class="text-dark" style="font-size: 0.88rem;">
                                <span class="d-inline-block rounded-circle me-2" style="width:8px; height:8px; background-color:{{ $r['warna'] }};"></span>
                                {{ $r['label'] }}
                            </span>
                            <span class="fw-bold font-monospace" style="font-size: 0.9rem;">{{ $r['jumlah'] }}</span>
                        </div>
                        <div class="progress mb-3" style="height:6px; background:#f1f5f9; border-radius:20px;">
                            <div class="progress-bar" style="width:{{ $totalSiswa ? round(($r['jumlah'] / $totalSiswa) * 100) : 0 }}%; background-color:{{ $r['warna'] }}; border-radius:20px;"></div>
                        </div>
                    @endforeach
                </div>
            </div>
